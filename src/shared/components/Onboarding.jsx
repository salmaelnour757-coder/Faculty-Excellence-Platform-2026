...
export default function Onboarding({ session, onComplete }) {
  ...
  const label = (text, required) => (
    <label style={{ display:'block', fontWeight:600, color:'var(--brand-primary)',
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--surface-page)',
      ...
      <div style={{ maxWidth:720, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:24,
          ...
          {STEPS.map(s => (
            <div key={s.id} style={{
              ...
              background: step === s.id ? 'var(--brand-primary)' : s.id < step ? 'var(--brand-accent)' : 'var(--border)',
              color: step === s.id || s.id < step ? 'white' : 'var(--text-secondary)',
            }}>
        ...
        {/* Card */}
        <div style={{ background:'var(--surface-card)', borderRadius:'var(--radius-control)',
          ...
          {step === 1 && (
            <div>
              <h3 style={{ color:'var(--brand-primary)', marginBottom:20 }}>🏛️ Institution Details</h3>
                  ...
                  {accredBodies.map(b => {
                    ...
                    return (
                      <button key={b} onClick={() => toggleAccred(b)} style={{
                        ...
                        background: sel ? 'var(--brand-accent)' : 'var(--border)',
                ...
                <div style={{ display:'flex', gap:16 }}>
                  {['primary','accent','gold'].map(key => (
                    <div key={key} style={{ display:'flex', alignItems:'center', gap:8 }}>
                      ...
                      <div>
                        <div style={{ fontSize:11, color:'var(--text-secondary)', textTransform:'capitalize' }}>{key}</div>
                        <div style={{ fontSize:12, fontWeight:700, color:'var(--brand-primary)' }}>{data.branding[key]}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {step === 2 && (
            <div>
              <h3 style={{ color:'var(--brand-primary)', marginBottom:8 }}>🗂️ Competency Framework</h3>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
                {['Teaching & Learning','Assessment & Feedback','Digital & AI Competence',
                  ...
                  <div key={i} style={{
                    ...
                    <span style={{
                      background:'var(--brand-accent)', color:'white', borderRadius:'50%',
                      width:24, height:24, display:'flex', alignItems:'center',
                      justifyContent:'center', fontSize:11, fontWeight:700, flexShrink:0
                    }}>{i+1}</span>
                    <span style={{ fontSize:12, color:'var(--brand-primary)', fontWeight:500 }}>{d}</span>
                  </div>
                ))}
              </div>
          ...
          {step === 3 && (
            <div>
              <h3 style={{ color:'var(--brand-primary)', marginBottom:8 }}>🏢 Your Profile</h3>
              <p style={{ color:'var(--text-secondary)', marginBottom:20, fontSize:13 }}>
          ...
          {step === 4 && (
            <div>
              <h3 style={{ color:'var(--brand-primary)', marginBottom:8 }}>⚙️ Platform Policies</h3>
              <p style={{ color:'var(--text-secondary)', marginBottom:20, fontSize:13 }}>
                ...
                <div key={item.key} style={{
                  ...
                  <button onClick={() => setPolicy(item.key, !data.policy[item.key])}
                    style={{
                      ...
                      background: data.policy[item.key] ? 'var(--brand-accent)' : 'var(--border)',
                  ...
                  <div>
                    <div style={{ fontWeight:700, color:'var(--brand-primary)', fontSize:14 }}>{item.label}</div>
                    <div style={{ fontSize:12, color:'var(--text-secondary)', marginTop:3 }}>{item.desc}</div>
                  </div>
                </div>
              ))}
              <div style={{ marginTop:16 }}>
                <div style={{ fontWeight:700, color:'var(--brand-primary)', fontSize:14, marginBottom:8 }}>
                  Portfolio Ownership
                </div>
                  ...
                  {['faculty','institution'].map(opt => (
                    <button key={opt} onClick={() => setPolicy('portfolio_ownership', opt)}
                      style={{
                        ...
                        background: data.policy.portfolio_ownership === opt ? 'var(--brand-primary)' : 'var(--border)',
                        color: data.policy.portfolio_ownership === opt ? 'white' : 'var(--text-secondary)',
          ...
          {step === 5 && (
            <div>
              <h3 style={{ color:'var(--brand-primary)', marginBottom:16 }}>🚀 Ready to Launch</h3>
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:24 }}>
                {[
                  ...
                  <div key={k} style={{
                    ...
                    <span style={{ fontWeight:700, color:'var(--brand-primary)', fontSize:13 }}>{v}</span>
                  </div>
                ))}
              </div>
              ...
              <button onClick={handleLaunch} disabled={loading}
                style={{
                  width:'100%', padding:14, background:loading ? 'var(--text-secondary)' : 'var(--brand-primary)',
